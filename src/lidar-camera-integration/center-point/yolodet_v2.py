...
OUTPUT_DIR = "yolo_depth_frames_all"
...
def remove_ground(points):
    """Strip points below the vehicle to avoid mapping depth to the road."""
...
def process_yolo_depth_frame(token, loader, yolo_model, trt_model):
    cam_name = "CAM_FRONT"
    ...
    for box, cls_id in zip(yolo_boxes, yolo_classes):
        ...
        if len(pts_in_box) > 0:
            ...
            if len(valid_semantic_pts) > 0:
                # SUCCESS: We have LiDAR points confirmed by the SegFormer mask!
                median_depth = np.median(depths[valid_semantic_pts])
                
                # Draw GREEN Box with Distance
                ...
                # Draw the valid LiDAR points in Red to visualize the depth sampling
                for pt_idx in valid_semantic_pts:
                    cv2.circle(blended, (int(pixels[pt_idx, 0]), int(pixels[pt_idx, 1])), 2, (0, 0, 255), -1)
                continue # Move to next box
                
        # FALLBACK: YOLO saw it, but LiDAR missed it (or Segformer rejected the points)
        # Draw BLUE Box without Distance
    ...
    cv2.putText(blended, "GREEN: Verified Distance | BLUE: YOLO Only", (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    return blended
...
if __name__ == "__main__":
    ...
    tokens = list(loader.samples.keys())

    for i, token in enumerate(tokens):
        print(f"[{i+1}/{len(tokens)}] Processing YOLO Depth Frame: {token[:8]}...")
        result_img = process_yolo_depth_frame(token, loader, yolo_model, trt_model)
        
        out_path = os.path.join(OUTPUT_DIR, f"yolo_depth_{i:03d}.jpg")
        cv2.imwrite(out_path, result_img)
    
    print(f"\n✅ YOLO Depth visualizations saved to {OUTPUT_DIR}")
